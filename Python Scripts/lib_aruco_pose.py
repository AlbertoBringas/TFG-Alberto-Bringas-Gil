...
class ArucoSingleTracker():
    ...
    def track(self, loop=True, verbose=False, show_video=None):
        ...
        while not self._kill:
            ...
            self._cap = client.simGetImage(self.camera_name, airsim.ImageType.Scene)
            font,fontFace,fontScale,thickness,textOrg = self.font_for_text()
            frame = cv2.imdecode(airsim.string_to_uint8_array(self._cap), cv2.IMREAD_UNCHANGED)

            self._update_fps_read()

            cv2.putText(frame,('FPS %.0f' %self.fps_read),textOrg, fontFace, fontScale,(255,0,255),thickness)
            ...
            if not ids is None and self.id_to_find in ids[0]:
                ...
                # aruco.drawDetectedMarkers(frame, corners) #DA ERROR
                aruco.drawAxis(frame, self._camera_matrix, self._camera_distortion, rvec, tvec, self.marker_size)
